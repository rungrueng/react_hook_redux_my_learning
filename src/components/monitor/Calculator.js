...
class Calculator extends Component {
    ...
    render() {
        const { key, totalPrice, orders } = this.props;
        return (
            <div>
                <h1 className="text-right">{totalPrice ? totalPrice + " ฿" : 0}</h1>
                <hr />
                ...
                <button className="btn btn-block btn-secondary title" onClick={() => this.props.onCancelOrder()}>{key}ยกเลิก</button>
            </div>
        );
    }
}
